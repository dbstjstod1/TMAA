...
## 라이브러리 추가하기
    ...
    parser.add_argument("--result_dir", default="./result_refine_sino_train", type=str, dest="result_dir")
    ...
    if mode == 'train':
        ...
        transform = transforms.Compose([ToTensor()])
        dataset_test = Dataset(os.path.join(data_dir_bh, 'train'), nu, nv, transform=transform)
    ...
    # TEST MODE
        ...
        with torch.no_grad():
            ...
            for batch, data in enumerate(loader_test, 1):
                ...
                # 손실함수 계산하기
                loss = fn_loss(output/10, label/10)
                #loss = fn_loss(input_bh*output, label)
                #loss = fn_loss(input*output, input*label)
                ...
                #input_p1 = np.reshape(input_bh.squeeze(), nu2*nv2)
