...
class conv_block(nn.Module):
    def __init__(self,ch_in,ch_out):
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(ch_in, ch_out, kernel_size=3,stride=1,padding=1,bias=True,padding_mode='replicate'),
            nn.BatchNorm2d(ch_out),
            nn.ReLU(inplace=True),
            nn.Conv2d(ch_out, ch_out, kernel_size=3,stride=1,padding=1,bias=True,padding_mode='replicate'),
            nn.BatchNorm2d(ch_out),
            nn.ReLU(inplace=True)
        )
...
class up_conv2(nn.Module):
    def __init__(self,ch_in,ch_out,image_size=[]):
        super(up_conv2,self).__init__()
        self.up = nn.Sequential(
            #nn.ConvTranspose2d(ch_in, ch_out, kernel_size=2, stride=2)
            nn.Upsample(size=image_size),
            #nn.Upsample(scale_factor=2),
            nn.Conv2d(ch_in,ch_out,kernel_size=3,stride=1,padding=1,bias=True,padding_mode='replicate'),
            nn.BatchNorm2d(ch_out),
            nn.ReLU(inplace=True)
...
class AttU_Net(nn.Module):
    def __init__(self,img_ch=1,output_ch=1):
        ...
        #self.Up5 = up_conv2(ch_in=1024,ch_out=512,image_size=[82,82])
        self.Up5 = up_conv(ch_in=1024,ch_out=512)
        self.Att5 = Attention_block(F_g=512,F_l=512,F_int=256)
        self.Up_conv5 = conv_block(ch_in=1024, ch_out=512)

        #self.Up4 = up_conv2(ch_in=512,ch_out=256,image_size=[165,165])
        self.Up4 = up_conv(ch_in=512,ch_out=256)
...
class Sparse_Net_comparison(nn.Module):
    def __init__(self,img_ch=1,output_ch=1):
        super(Sparse_Net_comparison,self).__init__()
...
class Sparse_ReconNet(nn.Module):
    def __init__(self,img_ch=128,output_ch=1):
        super(Sparse_ReconNet,self).__init__()
    ...
    def forward(self,x):
        ...
        d2 = torch.cat((x0,d2),dim=1) #cat initial view info

        d1 = self.Conv_1x1(d2)

        return d1
            
class AttU_Net_2ch(nn.Module):
    def __init__(self,img_ch=2,output_ch=1):
        super(AttU_Net_2ch,self).__init__()
