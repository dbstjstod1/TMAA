...
class UNetDec(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, reduction_ratio):
        ...
        self.up = nn.Sequential(
            ...
            nn.BatchNorm2d(out_channels),
            nn.ReLU(inplace=True),
            nn.ConvTranspose2d(out_channels, out_channels, 2, stride=2, padding= 0),
            nn.ReLU(inplace=True),
        )
        self.reduction_ratio = reduction_ratio
        if self.reduction_ratio is not None:
            self.excitation1 = nn.Conv2d(out_channels, out_channels // self.reduction_ratio, kernel_size=1, padding= 0)
            self.excitation2 = nn.Conv2d(out_channels // self.reduction_ratio, out_channels, kernel_size=1, padding= 0)
    ...
    def forward(self, x):
        ...
        if self.reduction_ratio:
            fm_size1 = out.size()[2]
            fm_size2 = out.size()[3]
            scale_weight = F.avg_pool2d(out,(fm_size1,fm_size2))
...
class UNet(nn.Module):
    ...
    def __init__(self, num_classes, init_features, network_depth, bottleneck_layers, reduction_ratio):
        ...
        self.final = nn.Conv2d(2 * init_features, num_classes, 1, stride=1, padding=0)
