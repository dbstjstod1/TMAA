...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, data_dir, nu, nv, transform=None):
        ...
        #self.nz = nz
        #self.nlam = nlam
        self.data_dir_input = os.path.join(self.data_dir, 'input_refine')
        self.data_dir_input2 = os.path.join(self.data_dir, 'residual_768')
        self.data_dir_label = os.path.join(self.data_dir, 'label_refine') 
    ...
    def __getitem__(self, index):
        ...
        input2 = np.fromfile(os.path.join(self.data_dir_input2, self.lst_input2[index]), dtype=np.float32).reshape([1, self.nv, self.nu])
        label = np.fromfile(os.path.join(self.data_dir_label, self.lst_label[index]), dtype=np.float32).reshape([1, self.nv, self.nu])
        
        data = {'input': input, 'input2': input2, 'label': label}
...
class ToTensor(object):
    def __call__(self, data):
        ...
        input, input2, label = data['input'], data['input2'], data['label']
      
        input = input.astype(np.float32)
        input2 = input2.astype(np.float32)
        label = label.astype(np.float32)
        
        data = {'input': torch.from_numpy(input), 'input2': torch.from_numpy(input2), 'label': torch.from_numpy(label)}
