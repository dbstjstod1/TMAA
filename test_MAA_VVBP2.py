...
## 라이브러리 추가하기
    ...
    parser.add_argument("--ckpt_dir", default="./checkpoint2", type=str, dest="ckpt_dir")
    
    parser.add_argument("--log_dir", default="./log2", type=str, dest="log_dir")
    parser.add_argument("--result_dir", default="./result2_test", type=str, dest="result_dir")
    
    parser.add_argument("--mode", default="train", type=str, dest="mode")
    parser.add_argument("--train_continue", default="off", type=str, dest="train_continue")
    parser.add_argument("--fine_tuning", default="on", type=str, dest="fine_tuning")
    parser.add_argument("--idx_num", default=0, type=int, dest="idx_num")
    ...
    _net = MAA_VVBP_test2(idx=idx).cuda()
    ...
    if mode == 'train':
        ...
        for epoch in range(st_epoch + 1, num_epoch + 1):
            ...
            for batch, data in enumerate(loader_train, 1):
                ...
                output, label = net(input,label)
    
                # backward pass
                optim.zero_grad()
    
                loss = fn_loss(output, label)
                #loss = fn_loss(input*output, label)
    ...
    # TEST MODE
        ...
        with torch.no_grad():
            ...
            for batch, data in enumerate(loader_test, 1):
                ...
                #input_cropped = fn_tonumpy(input_cropped)
                
                output = fn_tonumpy(output)

                id = batch 
        
                #f = open(os.path.join(result_dir, 'raw', 'label', 'label_%04d' % id + '_%d.raw' % idx), "wb")
                ...
                #f = open(os.path.join(result_dir, 'raw', 'input', 'input_%04d' % id + '_%d.raw' % idx), "wb")
                #input_p = np.reshape(input_cropped.squeeze(), nu2*nv2)
                #myfmt = 'f' * len(input_p)
                ...
                f = open(os.path.join(result_dir, 'raw', 'output', 'output_%04d' % id + '_%d.raw' % idx), "wb")
